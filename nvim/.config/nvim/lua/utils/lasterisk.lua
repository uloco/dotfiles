...
local function get_selected_text(mode)
	...
	return fn.join(vim.tbl_map(function(line)
...
M.search = function(opts)
	...
	if mode ~= "n" and mode ~= "V" and mode ~= "v" then
		api.nvim_echo(
			{ { "lasterisk: support normal, visual by character, visual by line only!", "WarningMsg" } },
			true,
			{}
		)
		return
	...
	if mode == "n" then
		...
		if cword == "" then
			api.nvim_echo({ { "lasterisk: No selected string", "WarningMsg" } }, true, {})
			return
		end
